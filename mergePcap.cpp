 ...
 * Title        : mergePcap.cpp
 *
 * Description  : read 2 .pcap files and merge them into 1 .pcap file with all the packets are sorted by time stamp
 *
 * Author       : X. Wang
 *
 ...
 * Thanks To    : Edison Xu
 *                Quanlong Liao
 *                Guy Harris on stackoverflow.com(http://stackoverflow.com/users/862787/guy-harris)
...
#include <vector>
#include <algorithm>
#include "mergePcap.h" // in which I define a struct to save each packet's header and data

using namespace std;
void sortPcap(pcap_t * spcap);
//void printData(pcap_t * pcap);
...
string tmpFile = "dont_timp.pcap"; // the file merged from file1 and file2 without sorting by timestamp
string sortedFile = "timp.pcap"; // the file already sorted by timestamp
...
/**
 * Method Name: pintData()
 * Arguments  : pcap_t *
 * Return Type: void
 * Description: print each packet of the .pcap file which is already read
 * Note       : this method can work efficiently, the program just doesn't need it at present
 */
//void printData(pcap_t * pcap) {
//
//	struct pcap_pkthdr * header; // header object
//	const u_char * data; // data object
//	u_int packetCount = 0; // counter used to count the No. of packets in a .pcap file
//	while (int returnValue = pcap_next_ex(pcap, &header, &data) >= 0) {
//
//		cout << "Packet # " << ++packetCount << endl; // print the packet No.
//		cout << "Time Point: "
//				<< header->ts.tv_sec * 1000000 + header->ts.tv_usec
//				<< "  u_seconds" << endl; // print the total time in u_seconds
//		cout << "Packet Size: " << header->len << "bytes" << endl; // print size of the packet in bytes
//
//		if (header->len != header->caplen) {
//			cout << "warning! capture size is different from packet size: "
//					<< header->len << "bytes" << endl; // throw a warning message in this condition
//		} else {
//			cout << "Epoch Time: " << header->ts.tv_sec << "."
//					<< header->ts.tv_usec << "   seconds" << endl; // print epoch time
//		}
//
